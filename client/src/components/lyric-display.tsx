...
export function LyricDisplay({
  ...
  // Mobile banner portal content - rendered to document.body to bypass scroll container issues in Android WebView
  const MobileBannerPortal = () => {
    if (!isMobile || !isPracticeMode) return null;
    
    return createPortal(
      <>
        {/* Listening Banner - Mobile Portal */}
        {isPracticeListening && (
          <div className="fixed top-0 left-0 right-0 z-[9999] px-3 pb-3 bg-background/95 backdrop-blur-sm border-b shadow-lg" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 9999, paddingTop: 'max(env(safe-area-inset-top, 48px), 48px)' }}>
  ...
  return (
    <Card className="p-6 h-[600px] overflow-hidden flex flex-col relative">
      {/* Mobile banners rendered via portal to bypass WebView fixed positioning issues */}
      <MobileBannerPortal />
