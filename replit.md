...
### Fixed Issues
- **Visual Card Share System - Tidal-Style with Logo Overlay (Nov 25, 2025)**: Enhanced share cards with Tidal-inspired design and official Lyric Sensei logo overlay on album artwork: (1) Updated generateStoryCard() to overlay logo image on top-left corner of album artwork (80×80px) with white rounded background, (2) Updated generateStoryCardCanvas() fallback to replicate same logo overlay with rounded white background, (3) Logo positioned at top-left of album artwork like Tidal, with 20px padding and subtle shadow, (4) Both html-to-image and Canvas implementations include white rounded background (96×96px with 12px radius) and logo image (80×80px with 8px padding), (5) Layout: purple blurred background → white card → album art with logo overlay → song/artist info → CTA. Cards now show professional Tidal-style branding with official Lyric Sensei logo, properly displaying when shared to Instagram Stories, Snapchat, and other social platforms.

- **Visual Card Share System (Nov 25, 2025 - CRITICAL FIX v2)**: FIXED Android share not showing images - root cause was navigator.share not properly handling File objects on Android WebView. Solution: (1) Switched from navigator.share (Web Share API) to Capacitor.Share API for mobile, which handles file URIs natively and reliably on Android/iOS, (2) Updated shareToInstagram(), shareToTwitter(), and shareToSocialMedia() to save generated image blobs to Capacitor's cache directory using Filesystem API, (3) Pass file URI (not File object) to Capacitor.Share.share(), (4) Added helper function blobToBase64 for proper blob conversion (fixes TypeScript Uint8Array iteration issue), (5) Canvas fallback still triggers if html-to-image fails due to Google Fonts CORS. Now when users click Share on mobile, images are saved to cache and shared via native Capacitor Share plugin, properly displaying image preview in Android share sheet before sending to Instagram/Twitter/etc.

- **Visual Card Share System (Nov 25, 2025 - CRITICAL FIX v1 ARCHIVED)**: Fixed major bug where share functionality was only sharing text instead of beautiful visual cards. Root cause: Old `social-share.ts` functions were never generating images. Solution: (1) Updated `shareToInstagram()` and `shareToTwitter()` in social-share.ts to generate 1080×1920 story cards using `generateStoryCard()` with canvas fallback, (2) Added albumArt parameter to ShareOptions interface, (3) Updated ShareMenu to receive and pass albumArt prop to social share functions, (4) Updated mobile-layout.tsx to pass currentSong.albumArt to ShareMenu. ISSUE: navigator.share doesn't properly handle File objects on Android WebView - upgraded to Capacitor.Share in v2.
