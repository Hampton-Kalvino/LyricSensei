...
- **Precise Lyrics Sync Calibration & Manual Scroll Control (Nov 25, 2025)**: Implemented timing offset calibration (-5s to +5s range) allowing users to adjust lyric sync by ±0.1s increments for precise timing. Added intelligent manual scroll detection that shows circular "Resume Auto-Scroll" button (floating action button style, bottom-right) when user manually scrolls during playback. Timing adjustment bar appears automatically when offset is active, featuring: (1) "-0.1s" button to shift lyrics earlier, (2) Current offset display (e.g., "+0.2s"), (3) "+0.1s" button to shift lyrics later, (4) Reset button to return to default. Auto-scroll is automatically disabled when user manually scrolls, preserving manual scroll position. Re-sync button re-enables auto-scroll with one tap. All controls maintain CSS layout integrity with backdrop-blur effects and smooth transitions. Works across web and mobile with full gesture support.
