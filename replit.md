...
- **Robust Capacitor File Sharing (Nov 25, 2025)**: Implemented enterprise-grade sharing using Capacitor Filesystem and Share APIs. New `ShareSheet.tsx` component with dialog-based UI matching Tidal design. Enhanced `story-generator.ts` now includes: `saveImageToFilesystem()` - persists PNG to device cache with proper file URI format, `shareToInstagramStory()` - native Share API with image files, `shareToSnapchat()` - Snapchat-optimized sharing, `shareToTwitter()` - platform-aware (web intents vs native), `shareMore()` - generic share sheet with fallback. Automatic platform detection: on native (Android/iOS) uses Capacitor APIs with file persistence, on web uses Web Share API. Proper error handling, logging, and graceful fallbacks when features unavailable. Uses `@capacitor/filesystem` and `@capacitor/share` for reliable native interop.
