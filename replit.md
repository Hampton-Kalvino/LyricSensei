...
- **Production-Grade Visual Story Cards (Nov 25, 2025)**: Replaced Capacitor Share API with `cordova-plugin-x-socialsharing` plugin. New `ShareSheet.tsx` generates beautiful 1080×1920px Tidal-style story cards with: Blurred album art background (80px blur, -50px offsets), 700×700px premium album frame with border/shadow, Bold 80px song title with text shadows, Professional 52px artist name, App branding tagline ("Learn Lyrics in Any Language"), Call-to-action footer ("Download Lyric Sensei + domain"). All functions: Generate card via `html-to-image` with 2x pixel ratio, Save PNG to device cache via Capacitor Filesystem, Pass file URI to `window.plugins.socialsharing`, Share to Instagram/Snapchat/Twitter with proper APIs. Features comprehensive logging, error handling with app-not-installed fallbacks, platform detection, and web browser support. Fixed "no content received" bug by using native plugin with proper image delivery.
