...
- **Production-Grade Social Sharing (Nov 25, 2025)**: Replaced Capacitor Share API with `cordova-plugin-x-socialsharing` plugin for reliable cross-platform sharing. New `ShareSheet.tsx` uses `window.plugins.socialsharing` for: `shareViaInstagramToStory()` - native Instagram Story sharing with background image, gradient colors, `shareVia()` - app-specific sharing (Snapchat, Twitter) with proper package names, `share()` - generic fallback with Web Share API. All functions generate story images via `html-to-image`, save to device cache via Capacitor Filesystem, then pass file URIs to native share API. Features comprehensive logging ([Instagram], [Snapchat], [Twitter] prefixes), error handling with fallbacks for when apps not installed, platform detection (Android/iOS), and web browser support via intents. Fixed bug where apps received no content by using native plugin instead of limited Capacitor API.
