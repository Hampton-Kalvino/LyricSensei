...
### Fixed Issues
- **Visual Card Share System - Tidal-Style with Lyric Sensei Logo (Nov 25, 2025)**: Enhanced share cards with Tidal-inspired design and official Lyric Sensei logo: (1) Updated generateStoryCard() to use actual logo image from /public/lyric-sensei-logo.png instead of SVG, (2) Updated generateStoryCardCanvas() fallback to load and render logo asynchronously, (3) Cleaner layout: purple blurred background → white card with rounded corners → Lyric Sensei logo (100×100px) → large album art → song/artist info → CTA, (4) Font sizing optimized: 56px title, 40px artist, 36px CTA, (5) Both html-to-image and Canvas fallback implementations match the new design. Cards now show professional Lyric Sensei branding with official logo, comparable to Tidal's share cards.
