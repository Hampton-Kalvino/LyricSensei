...
### Fixed Issues
- **Visual Card Share System - Tidal-Style Design (Nov 25, 2025)**: Enhanced share cards with Tidal-inspired design: (1) Updated generateStoryCard() and generateStoryCardCanvas() to use white card background (like Tidal), (2) Added purple gradient circle logo at top with white music note icon, (3) Cleaner layout: purple blurred background → white card with rounded corners → purple logo circle with music note → large album art → song/artist info → CTA, (4) Font sizing optimized: 56px title, 40px artist, 36px CTA, (5) Both html-to-image and Canvas fallback implementations match the new design. Cards now look professional and branded with the Lyric Sensei logo, comparable to Tidal's share cards.
